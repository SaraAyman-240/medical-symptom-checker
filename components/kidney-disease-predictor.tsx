...
export function KidneyDiseasePredictor() {
  ...
  const handleCalculate = async () => {
    ...
    // Build payload expected by FastAPI (snake_case)
    const payload = {
      sex: formData.sex,
      age: formData.age,
      ...
      rbg: Number(formData.bloodGlucoseRandom),
      ...
      wbcc: Number(formData.whiteBloodCellCount),
      rbcc: Number(formData.redBloodCellCount),
      htn: formData.hypertension === "yes",
      dm: formData.diabetes === "yes",
      ane: formData.anemia === "yes",
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE.replace(/\/$/, "")}/api/kidney/predict`, // remove /api if your backend isn't prefixed
        {
          method: "POST",
          headers: {
          ...
          // If using cookie auth instead of Authorization header:
          // credentials: "include",
      ...
      const mapped: RiskResult = {
        riskLevel:
          data.risk_level === "high"
            ...
            ? "Moderate"
            : "Low",
        riskPercentage: Math.round((data.risk_score ?? 0) * 100),
